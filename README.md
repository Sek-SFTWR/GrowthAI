...
How It Works
Data Collection: The system uses historical student data, which may include past grades, attendance, extracurricular activities, and demographic information.
Model Training: A CNN model is trained on the collected data to recognize patterns and predict future student performance.
Prediction: Once the model is trained, it can be deployed on a Flask server. The user enters relevant student data, and the system predicts the student's future grade.
Evaluation: Model performance is evaluated using standard metrics like accuracy, precision, and recall.
